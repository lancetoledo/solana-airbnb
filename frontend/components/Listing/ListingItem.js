...
function ListingItem({ id, connected, showReservedListing, location, distance, price, rating, imageURL, isReserved, reservation, removeListing, openEditListingModal, openReserveListingModal }) {
    ...
    return (
        <div className="flex flex-col space-y-3 cursor-pointer">
            <div className="relative h-64 w-auto group">
                ...
                {connected && (
                    <div className="opacity-0 group-hover:opacity-100 transition-all duration-150 absolute top-4 right-4 flex space-x-2">
                        <PencilIcon onClick={() => openEditListingModal(id)} className="w-6 h-6 text-white opacity-80" />
                        <TrashIcon onClick={() => removeListing(id)} className="w-6 h-6 text-white opacity-80" />
                        <HeartIcon onClick={() => openReserveListingModal(id)} className={`w-6 h-6 text-white  ${isReserved ? 'fill-red-500' : 'opacity-80'}`} />
                    </div>
                )}
            </div>
            ...
            <div>
                ...
                <p className="text-sm font-light text-gray-600">{formatNumber(distance.km)} kilometers</p>
                ...
                <p className="text-sm font-light text-gray-800 mt-2">
                    <span className="text-base font-medium">RM {formatNumber(price.perNight)}</span>
                    &nbsp;night
                </p>
            </div>
        </div>
